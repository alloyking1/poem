...
<script>
    /*
    * make api call to execute like system
    * @param post ID
    * @return response
    */
    function myLike(id){
        fetch('/api/user/post/like/'+id, {headers:new Headers({'Accept':'text/plain','X-Your-Custom-Header':'example value'})})
        .then(response =>{
    ...
    /**Fetch all comment related to a post
    @param post ID
    ...
    function fetchComment(id){
        ...
        fetch('/api/user/poem/comment/fetch/'+id, {headers:new Headers({'Accept':'text/plain','X-Your-Custom-Header':'example value'})})
        .then(res => {
            return res.json();
        })
        .then(json => {
            let commentArray = json.poem_comment;
            console.log(json)
            // for(var i = 0, length = commentArray.length; i < length; i++){
            //     console.log(commentArray[i].comment);

            //     var elemP = document.createElement("P");
            //     var commentNode = document.createTextNode(commentArray[i].comment);
            //     elemP.appendChild(commentNode);
